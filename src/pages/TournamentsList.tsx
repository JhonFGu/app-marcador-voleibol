...
export default function TournamentsList() {
  ...
  const getStatusBadge = (status: Tournament['status']) => {
    if (status === 'active') {
      return (
        <span className="px-2 py-0.5 text-xs font-bold rounded bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 uppercase tracking-wide">
          En Vivo 🔴
        </span>
      );
    }
    return (
      <span className="px-2 py-0.5 text-xs font-bold rounded bg-blue-500/10 text-blue-500 border border-blue-500/20 uppercase tracking-wide">
        Finalizado
      </span>
    );
  };
  ...
  return (
    <div className="flex flex-col min-h-screen bg-black text-white p-4 font-sans select-none relative pb-10">
      ...
      <div className="flex items-center gap-3 mb-6 py-3 border-b border-zinc-900 bg-zinc-950/40 px-3 rounded-2xl">
        ...
        <div>
          <h2 className="text-[22px] font-extrabold tracking-tight">Torneos Activos</h2>
          <p className="text-sm text-gray-400">PuntosVolley online</p>
        </div>
      </div>
      ...
      <div className="flex-grow flex flex-col gap-6 max-w-md mx-auto w-full">
        {loading ? (
          ...
          <div className="p-10 border border-zinc-900 border-dashed rounded-3xl text-center flex flex-col items-center justify-center bg-zinc-950/20 my-auto">
            ...
            <h4 className="text-base font-bold text-zinc-400 mb-1">Sin torneos activos</h4>
              ...
              >
                <div className="flex flex-col gap-1 text-left max-w-[75%]">
                  <div className="flex items-center gap-2 flex-wrap">
                    <h4 className="font-extrabold text-base text-zinc-150 group-hover:text-purple-brand transition-colors truncate max-w-[170px]">{t.name}</h4>
